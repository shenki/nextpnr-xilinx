...
std::vector<Arch::ConstHoldout> Arch::routeVcc()
{
    ...
    // Sinks the backbone fill can't reach are returned to routeConstants(),
    // which drives them from a local constant LUT and re-routes (see
    // route_const.cc).  NEXTPNR_GND_HOLDOUT_FILE additionally dumps the GND
    // ones, one line per holdout: "<cell> <port>" (e.g. "mem_addr_reg_13__i_2 DI0").
...
bool Arch::route()
{
    ...
    // Whatever the fill cannot reach (a signal net holding the only GFAN/bounce
    // wire into a LUT pin) is driven from a local constant LUT and re-routed by
    // the main router; routeConstants() fails loudly if anything is still left.
