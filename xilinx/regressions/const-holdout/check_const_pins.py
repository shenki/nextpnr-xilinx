...
$PACKER_GND_NET$holdout$N drivers the fixed constant router adds) whose net
reaches the site pin.

An unrouted xc7 input mux reads 1, so a missing pin means the address bit
floats high and the memory is silently corrupted.  Exit status 1 when any
pin is missing or no LUTRAM was found at all.
"""
import collections
import json
import sys
